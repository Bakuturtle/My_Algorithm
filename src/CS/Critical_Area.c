...
void* mythread_r(void* ptr); //void 포인터는 주소값을 담는 역할을 한다. 그렇기에 형변환 자체는 자유롭다.
...
//홀짝 출력이 이상하게 나오는 이유는 mythread_w하고 mythread_r이 임계지역 충돌을 일으켜서 
// mythread_r에서 홀짝 연산후에 mythread_w에서 증감을 시키는 등 충돌이 일어났기 때문인거같다.
